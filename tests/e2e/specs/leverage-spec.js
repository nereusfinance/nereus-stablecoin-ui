...
describe("Leverage tests", () => {
    before(() => {
        cy.addMetamaskNetwork({
            ...
            rpcUrl:
                "https://rpc.tenderly.co/fork/7d815d93-c3c7-482f-a4e2-67745aa5c5ba",
            chainId: "43114",
            symbol: "AVAX",
            blockExplorer: "",
            isTestnet: true,
    ...
    context("Leverage", () => {
        it(`should change borrow amount to max`, () => {
            ...
            cy.get("[data-cy=available-borrow]").then((availableBorrow) => {
                ...
                expect(Math.abs(Number(input.val())-availableBorrowVal)).to.be.lessThan(1)
                })
            });
        });
            ...
            cy.get("[data-cy=available-borrow]").then((availableBorrow) => {
                ...
                cy.get("[data-cy=expected-leverage]").then((expected) => {
                    ...
                    cy.get("[data-cy=borrowed-value]").then((borrowed) => {
                        ...
                        expect(Math.abs(Number(expected.text()) - borrowedVal)).to.be.lessThan(1)
                    })
