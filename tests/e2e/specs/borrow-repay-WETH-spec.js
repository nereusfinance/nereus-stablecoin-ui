...
const poolId = 8;
describe("Borrow-repay WETH tests", () => {
  before(() => {
  ...
  context("Borrow screen for WETH", () => {
    it(`should add WETH as collateral`, () => {
      tradePage.visit(poolId);
    ...
    it(`should borrow NXUSD and add WETH as collateral`, () => {
      tradePage.visit(poolId);
      cy.get("[data-cy=borrowed-value]").then((borrowed) => {
        cy.get("[data-cy=collateral-deposited]").then((deposited) => {
          const borrowedVal = Number(borrowed.text().split(" ")[2]);
  ...
  context("Repay screen", () => {
    ...
    it(`should remove WETH collateral`, () => {
      tradePage.visit(poolId);
      tradePage.clickRepay();
      cy.get("[data-cy=collateral-deposited]").then((deposited) => {
    ...
    it(`should repay NXUSD and remove WETH collateral`, () => {
      tradePage.visit(poolId);
      tradePage.clickRepay();
      cy.get("[data-cy=borrowed-value]").then((borrowed) => {
        cy.get("[data-cy=collateral-deposited]").then((deposited) => {
